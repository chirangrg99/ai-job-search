...
Reproduced the expanded-source failure: generated evidence stitched headings/bullets and changed source wording. Replaced generated quotations at the model boundary with numbered source references, resolved by application code. The hosted source renderer now exposes literal decoded text, preserving strict database evidence checks. Added reference, whitespace, scalar rejection and database rendering regression coverage. No candidate data is sent and no future-phase feature is introduced.

User-authorized live calls reproduced failures and then produced validated full-posting output. Final output correctly leaves schedule requirements empty for hybrid city-presence wording. Audit still found missed bilingual experience-threshold conflicts; the result is source-grounded but not guaranteed semantically exhaustive. No claim of perfect classification is made.

Final verification: 515 tests pass across 36 files; lint, typecheck, formatting and production build pass. Hosted PostgreSQL accepted the live extraction through its literal evidence validator, and the v7 result was saved using the owner-scoped reservation flow. Restarted the updated production preview on port 3001. Interactive browser verification was unavailable in this session.
